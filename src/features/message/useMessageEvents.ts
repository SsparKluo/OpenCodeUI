...
import type { ApiPart, ApiPermissionRequest, ApiQuestionRequest, ApiMessage } from '../../api'
    ...
    const unsubscribe = subscribeToEvents({
      ...
      onMessageUpdated: (apiMsg: ApiMessage) => {
        ...
        callbacksRef.current.setMessages(prev => {
          ...
          if (existing) {
            // 更新现有消息的 info
            return prev.map(m => (m.info.id === apiMsg.id ? { ...m, info: apiMsg as MessageInfo } : m))
          }
      ...
      onSessionIdle: data => {
        ...
        callbacksRef.current.setMessages(prev => prev.map(m => (m.isStreaming ? { ...m, isStreaming: false } : m)))
      ...
      onPermissionReplied: data => {
        if (data.sessionID !== currentSessionIdRef.current) return
        callbacksRef.current.setPendingPermissionRequests(prev => prev.filter(r => r.id !== data.requestID))
      },
      ...
      onQuestionReplied: data => {
        if (data.sessionID !== currentSessionIdRef.current) return
        callbacksRef.current.setPendingQuestionRequests(prev => prev.filter(r => r.id !== data.requestID))
      },
